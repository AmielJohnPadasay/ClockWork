...
class MainApp:
    def __init__(self):
        ...
        self.stacked_supervisor = self.dashboard_window.findChild(QStackedWidget, "stacked_Supervisor")
        self.dashboard_btn = self.dashboard_window.findChild(QWidget, "dashboard_btn")
        if self.dashboard_btn:
            self.dashboard_btn.clicked.connect(lambda: self.stacked_supervisor.setCurrentIndex(0))
        
        self.activity_log_btn = self.dashboard_window.findChild(QWidget, "activity_log_btn")
        if self.activity_log_btn:
            self.activity_log_btn.clicked.connect(lambda: self.stacked_supervisor.setCurrentIndex(1))
        
        self.calendar_btn = self.dashboard_window.findChild(QWidget, "calendar_btn")
        if self.calendar_btn:
            self.calendar_btn.clicked.connect(lambda: self.stacked_supervisor.setCurrentIndex(2))
        
        self.group_btn = self.dashboard_window.findChild(QWidget, "groups_btn")
        if self.group_btn:
            self.group_btn.clicked.connect(lambda: self.stacked_supervisor.setCurrentIndex(3))

        # Manager's Dashboard
        self.stacked_Manager = self.dashboard_window.findChild(QStackedWidget, "stacked_Manager")
        self.dashboard_btn = self.dashboard_window.findChild(QWidget, "dashboard_btn")
        if self.dashboard_btn:
            self.dashboard_btn.clicked.connect(lambda: self.stacked_Manager.setCurrentIndex(0))
        
        self.activity_log_btn = self.dashboard_window.findChild(QWidget, "activity_log_btn")
        if self.activity_log_btn:
            self.activity_log_btn.clicked.connect(lambda: self.stacked_Manager.setCurrentIndex(1))
        
        self.calendar_btn = self.dashboard_window.findChild(QWidget, "calendar_btn")
        if self.calendar_btn:
            self.calendar_btn.clicked.connect(lambda: self.stacked_Manager.setCurrentIndex(2))
        
        self.group_btn = self.dashboard_window.findChild(QWidget, "groups_btn")
        if self.group_btn:
            self.group_btn.clicked.connect(lambda: self.stacked_Manager.setCurrentIndex(3))
        """
